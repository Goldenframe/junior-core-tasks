...
export const validateJobTitle = (value: string) =>
  validators.jobTitle.test(value);
export const validateInterests = (value: string) =>
  validators.interests.test(value);
...
export const validateEmployeeForm = (formData: {
  ...
  if (!validatePhone(formData.phone_number)) {
    errors.phone_number =
      "Телефон должен быть в формате: 1234567890 или 123-456-7890 или 123 456 7890";
  }

  if (!validateJobTitle(formData.job_title)) {
    errors.job_title =
      "Должность должна содержать только буквы (2-50 символов)";
  }

  if (!validateInterests(formData.interests)) {
    errors.interests =
      "Интересы должны содержать буквы, цифры, пробелы, запятые (2-200 символов)";
  }
